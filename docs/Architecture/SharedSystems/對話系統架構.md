...
│
├── 🔄 養成模式對話系統
│   ├── TextBoxTestPlaying.cs (核心對話控制器)
│   ├── GameManagerTest.cs (養成場景管理器)
│   ├── PlayerActorTest.cs (玩家互動觸發器)
│   │   ├── Interactive_Speak() - 普通對話
│   │   ├── Interactive_FlirtTalk() - 情話互動
│   │   ├── Interactive_Operate() - 工作對話
│   │   └── 其他互動方法 (12+ 種互動類型)
│   ├── ActorManagerTest.cs (簡單表情控制)
│   └── 數據源: dialog[FDS_LV] 
│
├── 🎬 劇情模式對話系統  
│   ├── TextBoxDrama.cs (豐富對話控制器)
│   ├── GamePlayingManagerDrama.cs (劇情場景管理器)
│   ├── PlayerCtrlDrama.cs (劇情流程控制)
│   ├── ActorManagerDrama.cs (複雜表情與動畫控制)
│   ├── CGDisplay.cs (CG場景展示)
│   └── 數據源: diaData[DramaNumber] (1-9+ 劇情分支)
│
└── 🔗 共通系統
    ├── DialogData.cs (對話數據結構)
